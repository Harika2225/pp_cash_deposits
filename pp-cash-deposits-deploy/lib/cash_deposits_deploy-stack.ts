...
export class CashDepositsDeployStack extends cdk.Stack {
  constructor(
    scope: Construct,
    id: string,
    props: CashDepositsDeployStackProps
  ) {
    ...
    const taskDefinition_cashdeposits = new ecs.FargateTaskDefinition(
      this,
      `${prefix}-${appName}-${env}-taskdef`,
      {
    ...
    const container = taskDefinition_cashdeposits.addContainer(
      `${prefix}-${appName}-${env}`,
      {
        image: image,
    ...
    const new_uiService = new ecs.FargateService(
      this,
      `${prefix}-${appName}-${props.containerName}`,
      {
        cluster: cluster,
        assignPublicIp: true,
        desiredCount: 1,
    ...
    new elbv2.ApplicationListenerRule(this, "MyApplicationListenerRule1", {
      ...
      conditions: [
        elbv2.ListenerCondition.hostHeaders(["manage.dev.premiumparking.com"]),
        elbv2.ListenerCondition.pathPatterns(["/cash_deposits/*"]),
      ],
    });
